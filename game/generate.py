...
class Map:
    ...
    def ldraw(self, a: Edge, b: Edge) -> bool:
        ...
        ap = a.mid
        bp = b.mid
        cross = Int2(bp.x, ap.y) if a.is_vert else Int2(ap.x, bp.y)
        print(f"###################### FROM {ap} -> {cross} -> {bp}")
        ...
        while ap != cross:
            if not self.check_all(ap, ap + a.delta, ap - a.delta):
                return False
            points.append(ap)
            ap += a.norm
        bp += b.norm
        points.append(cross)
        while bp != cross:
            if not self.check_all(bp, bp + b.delta, bp - b.delta):
                return False
            points.append(bp)
            bp += b.norm
        for p in points:
    ...
    def sdraw(self, e0: Edge, e1: Edge) -> bool:
        e0 = e0.shrink(1)
        e1 = e1.shrink(1)
        proj0 = project(e0, e1)
        proj1 = project(e1, e0)
    ...
    def join_rects(self, a: Rect, b: Rect) -> bool:

        # First check if we can draw a straight corridor
        if self.sdraw(a.top_edge, b.bottom_edge):
            return True
        if self.sdraw(a.left_edge, b.right_edge):
            return True
        if self.sdraw(a.bottom_edge, b.top_edge):
            return True
        if self.sdraw(a.right_edge, b.left_edge):
            return True

        # Now check L-shaped corridors
